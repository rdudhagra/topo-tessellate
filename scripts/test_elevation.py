...
def test_elevation_map(bounds, topo_dir="topo", save_plots=True):
    ...
    print(f"Region: Longitude {min_lon}° to {max_lon}°, Latitude {min_lat}° to {max_lat}°")
    ...
    srtm_files = [f for f in os.listdir(topo_dir) if f.endswith('.hgt.zip') or f.endswith('.hgt')]
    ...
    try:
        ...
        print(f"Elevation data generation completed in {end_time - start_time:.2f} seconds")
        ...
        if zero_count > 0:
            print(f"Warning: {zero_count} zero elevation points found (possible no-data values)")
...
def create_elevation_plots(elevation_data, bounds, save_plots=True):
    ...
    contour = axes[1, 1].contour(elevation_data, levels=20, colors='brown', linewidths=0.5)
    contour_filled = axes[1, 1].contourf(elevation_data, levels=20, cmap='terrain', alpha=0.8)
