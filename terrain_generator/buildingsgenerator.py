...
class BuildingsGenerator:
    ...
    def generate_buildings(
        ...
        buildings_mesh_group = mr.Mesh()

        for i, building in enumerate(buildings):
            if i % 1000 == 0:
                output.progress_info(
                    f"Generating building {i+1}/{len(buildings)}"
                )
            ...
            building_mesh.transform(
                mr.AffineXf3f.translation(
                    mr.Vector3f(0, 0, foundation_elevation + height)
                )
            )
            ...
            # Add the building mesh to the buildings mesh group
            buildings_mesh_group.addMeshPart(mr.MeshPart(building_mesh))

        return buildings_mesh_group
